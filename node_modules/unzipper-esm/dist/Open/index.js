...
var _directory = _interopRequireDefault(require("./directory.js"));
...
function _asyncToGenerator(n) { return function () { var t = this, e = arguments; return new Promise(function (r, o) { var a = n.apply(t, e); function _next(n) { asyncGeneratorStep(a, r, o, _next, _throw, "next", n); } function _throw(n) { asyncGeneratorStep(a, r, o, _next, _throw, "throw", n); } _next(void 0); }); }; } // `fs` was replaced with `graceful-fs` as a workaround for an error
// when extracting `.zip` file contents into a directory:
// "Error: EMFILE: too many open files".
// https://github.com/ZJONSSON/node-unzipper/issues/149#issuecomment-527616280
//
// The reason for the `EMFILE` error seems to be that calling `Open.file(...)`
// attempts to read the list of all the files in the archive, and it does so
// by calling `fs.createReadStream(pathToArchive, { start, end })` in order to
// only read the individual files' headers without reading those files' contents.
//
// And, if there're too much files in an archive, it supposedly results in
// too may "open file descriptors" being created, while there's a limit on those
// that is imposed by the Operating System. For example, on Linux it's 1024 per process (by default).
//
// 'node:stream'
var _default = exports["default"] = {
  buffer: function buffer(_buffer, options) {
    var source = {
      stream: function stream(offset, length) {
        ...
        stream.end(_buffer.slice(offset, end));
        return stream;
      },
      size: function size() {
    ...
    return (0, _directory["default"])(source, options);
  },
  file: function file(filename, options) {
    var source = {
      stream: function stream(start, length) {
        var end = length ? start + length : undefined;
        return _gracefulFs["default"].createReadStream(filename, {
          start: start,
          end: end
        });
      },
      size: function size() {
        return new Promise(function (resolve, reject) {
          _gracefulFs["default"].stat(filename, function (err, d) {
  ...
  s3_v3: function s3_v3(client, params, options) {
    ...
    //
    var _global = global,
      GetObjectCommand = _global.GetObjectCommand,
      HeadObjectCommand = _global.HeadObjectCommand;
    ...
    var source = {
      size: function () {
        var _size = _asyncToGenerator(/*#__PURE__*/_regenerator().m(function _callee() {
          var head;
          return _regenerator().w(function (_context) {
            while (1) switch (_context.n) {
              ...
              case 2:
                return _context.a(2, head.ContentLength);
            }
          }, _callee);
        }));
        function size() {
      ...
      stream: function stream(offset, length) {
        ...
        client.send(new GetObjectCommand({
          Bucket: params.Bucket,
          Key: params.Key,
          Range: "bytes=".concat(offset, "-").concat(end)
  ...
  custom: function custom(source, options) {
    return (0, _directory["default"])(source, options);
  }
};
