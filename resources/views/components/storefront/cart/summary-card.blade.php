...
<aside class="rounded-[28px] border border-slate-200 bg-white p-5 shadow-card lg:sticky lg:top-32 lg:p-6">
    <div class="flex items-start justify-between gap-4">
        <div>
            <p class="text-xs font-black uppercase tracking-[.18em] text-brand-red">Order summary</p>
            <h2 class="mt-1 font-display text-3xl font-bold uppercase text-brand-ink">Cart Total</h2>
        </div>
    ...
    @if ($cart['coupon_code'])
        <div class="mt-5 flex items-center justify-between gap-3 rounded-2xl border border-green-200 bg-green-50 p-3 text-sm">
        ...
        <form method="POST" action="{{ route('cart.coupon.apply') }}" class="mt-5">
            ...
            <div class="mt-2 flex gap-2">
                <input
                    ...
                    type="text"
                    placeholder="TEAM10"
                    class="h-12 min-w-0 flex-1 rounded-xl border border-slate-300 bg-slate-50 px-3 text-sm font-bold uppercase text-slate-800 outline-none focus:border-brand-blue"
                >
                <button class="btn btn-light" type="submit">Apply</button>
            </div>
            <p class="mt-2 text-xs font-semibold text-slate-500">Demo coupons: TEAM10 or NEXTPLAY25.</p>
        </form>
    @endif
    ...
    <div class="mt-6 rounded-2xl bg-brand-navy p-4 text-white">
        <div class="flex items-end justify-between gap-4">
            ...
            <span class="font-display text-4xl font-bold">${{ number_format($cart['total'], 2) }}</span>
        </div>
        <p class="mt-2 text-xs font-semibold leading-5 text-blue-100">Final checkout totals must be recalculated by the backend before payment.</p>
    </div>
    ...
    <div class="mt-5 flex flex-wrap items-center gap-2 border-t border-slate-100 pt-4">
        @foreach ($cart['payment_badges'] as $badge)
            <span class="rounded-full border border-slate-200 bg-slate-50 px-3 py-1 text-[11px] font-black uppercase tracking-wide text-slate-500">{{ $badge }}</span>
        @endforeach
    </div>
</aside>
