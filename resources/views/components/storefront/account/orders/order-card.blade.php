    ...
    <div class="grid gap-5 p-5 md:grid-cols-[1fr_auto] md:items-center">
        <div class="flex -space-x-3">
            @foreach($order->items->take(4) as $item)
                <img src="{{ \App\Support\PublicMedia::url(null, $item->image_url, '/images/product-placeholder.svg') }}" alt="{{ $item->product_name }}" class="h-14 w-14 rounded-xl border-2 border-white bg-white object-contain" loading="lazy">
            @endforeach
        </div>
