        ...
        <div class="mt-5 space-y-3">
            <template x-for="(image, index) in imageUrls" :key="index">
                <div class="grid gap-3 rounded-2xl border border-slate-200 p-4 md:grid-cols-[minmax(0,1.5fr)_minmax(0,1fr)_auto_auto] md:items-end">
                    ...
                    <div><input type="hidden" :name="`image_urls[${index}][is_primary]`" :value="image.is_primary ? 1 : 0"><button type="button" class="btn btn-white whitespace-nowrap" @click="setPrimaryImage(index)" x-text="image.is_primary ? 'Primary image' : 'Make primary'"></button></div>
