@php($aspect = \App\Support\HomepageImageAspectRatios::forSectionImage('season_sale'))
<x-admin.homepage.section-panel title="Season Sale" description="Control every editable text and the responsive banner image in the sale section.">
    ...
    <div class="mt-5">
        <x-admin.homepage.media-field label="Banner image" :image="$viewSection['image']" :image-url="$viewSection['image_url']" :image-alt="$viewSection['image_alt']" :aspect-ratio="$aspect['ratio']" :aspect-tolerance="(int) round($aspect['tolerance'] * 100)" />
    </div>
