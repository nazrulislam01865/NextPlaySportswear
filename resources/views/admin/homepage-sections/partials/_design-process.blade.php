@php($aspect = \App\Support\HomepageImageAspectRatios::forSectionItem('design_process'))
    ...
    <div class="mt-5 space-y-4">
        @foreach($viewSection['items'] as $index => $item)
            <x-admin.homepage.item-card :title="str_pad((string)($index + 1), 2, '0', STR_PAD_LEFT).' · '.($item['title'] ?? 'Step')">
                ...
                <div class="mt-4"><x-admin.homepage.media-field label="Step image" prefix="items[{{ $index }}]" :image="$item['image'] ?? null" :image-url="$item['image_url'] ?? null" :image-alt="$item['image_alt'] ?? null" :aspect-ratio="$aspect['ratio']" :aspect-tolerance="(int) round($aspect['tolerance'] * 100)" /></div>
