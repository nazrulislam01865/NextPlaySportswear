    ...
    <div class="space-y-4">
        @foreach($viewSection['items'] as $index => $item)
            <x-admin.homepage.item-card :title="$item['title'] ?? strtoupper($item['id'] ?? 'Tile')">
                ...
                <div class="mt-4"><x-admin.homepage.media-field label="Tile image" prefix="items[{{ $index }}]" :image="$item['image'] ?? null" :image-url="$item['image_url'] ?? null" :image-alt="$item['image_alt'] ?? null" /></div>
