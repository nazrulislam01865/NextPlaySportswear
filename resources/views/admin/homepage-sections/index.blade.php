<x-layouts.admin
    ...
    eyebrow="Storefront"
    subtitle="Manage homepage visibility, order, content, and storefront display controls."
    :storefront-url="route('home')"
>
    @php
        $testimonialsSection = collect($sections)->firstWhere('key', 'testimonials') ?? [];
        $showStatistics = (bool) data_get($testimonialsSection, 'settings.show_statistics', true);
        $showFallbackRatings = (bool) data_get($testimonialsSection, 'settings.show_default_product_ratings', config('storefront.product_cards.show_default_rating', true));
    @endphp
    ...
    <div class="space-y-6">
        ...
        <div class="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm sm:p-6">
            <div class="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
                <div>
                    <p class="text-xs font-black uppercase tracking-[.2em] text-brand-red">Storefront Display Controls</p>
                    <h3 class="mt-1 text-xl font-black text-brand-ink">Ratings & testimonial statistics</h3>
                    <div class="mt-3 flex flex-wrap gap-2">
                        <span @class([
                            'rounded-full px-3 py-1 text-xs font-black',
                            'bg-emerald-50 text-emerald-700' => $showStatistics,
                            'bg-slate-100 text-slate-500' => ! $showStatistics,
                        ])>Testimonial statistics: {{ $showStatistics ? 'Shown' : 'Hidden' }}</span>
                        <span @class([
                            'rounded-full px-3 py-1 text-xs font-black',
                            'bg-emerald-50 text-emerald-700' => $showFallbackRatings,
                            'bg-slate-100 text-slate-500' => ! $showFallbackRatings,
                        ])>Fallback product ratings: {{ $showFallbackRatings ? 'Shown' : 'Hidden' }}</span>
                    </div>
                </div>
                <a href="{{ route('admin.homepage.sections.edit', 'testimonials') }}#storefront-display-controls" class="btn btn-red">Manage Display Controls</a>
            </div>
        </div>
