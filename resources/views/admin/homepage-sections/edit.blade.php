...
>
    ...
    >
        ...
        @if($section->key === 'testimonials')
            <div id="storefront-display-controls" class="scroll-mt-24">
                <x-admin.section-card title="Storefront Display Controls" description="Control whether testimonial statistics and fallback product ratings are visible to storefront customers.">
                <div class="grid gap-4 lg:grid-cols-2">
                    <label class="flex items-start gap-3 rounded-xl border border-slate-200 bg-white px-4 py-4">
                        <input type="hidden" name="settings[show_statistics]" value="0">
                        <input
                            type="checkbox"
                            name="settings[show_statistics]"
                            value="1"
                            @checked((bool) old('settings.show_statistics', data_get($viewSection, 'settings.show_statistics', true)))
                            class="mt-0.5 h-5 w-5 rounded border-slate-300 text-brand-red focus:ring-brand-red"
                        >
                        <span>
                            <strong class="block text-sm font-black text-brand-ink">Show testimonial statistics</strong>
                            <span class="mt-1 block text-xs font-medium leading-5 text-slate-600">Shows the 4.9/5, 96%, 500+ teams, and 24h statistics below the testimonial cards.</span>
                        </span>
                    </label>

                    <label class="flex items-start gap-3 rounded-xl border border-slate-200 bg-white px-4 py-4">
                        <input type="hidden" name="settings[show_default_product_ratings]" value="0">
                        <input
                            type="checkbox"
                            name="settings[show_default_product_ratings]"
                            value="1"
                            @checked((bool) old('settings.show_default_product_ratings', data_get($viewSection, 'settings.show_default_product_ratings', config('storefront.product_cards.show_default_rating', true))))
                            class="mt-0.5 h-5 w-5 rounded border-slate-300 text-brand-red focus:ring-brand-red"
                        >
                        <span>
                            <strong class="block text-sm font-black text-brand-ink">Show fallback product ratings</strong>
                            <span class="mt-1 block text-xs font-medium leading-5 text-slate-600">When enabled, products without real review data can show the configured default rating and review count. Real product ratings are not affected.</span>
                        </span>
                    </label>
                </div>
                </x-admin.section-card>
