                ...
                <tbody class="divide-y divide-slate-100">
                    @forelse($slides as $slide)
                        ...
                        <tr class="align-top">
                            ...
                            <td class="px-5 py-4"><span class="rounded-full px-2.5 py-1 text-xs font-black {{ $slide->show_content ? 'bg-blue-50 text-blue-700' : 'bg-slate-100 text-slate-600' }}">{{ $slide->show_content ? 'Text visible' : 'Image only' }}</span><p class="mt-2 max-w-xs line-clamp-2 text-xs text-slate-500">{{ $slide->description }}</p></td>
                            ...
                            <td class="px-5 py-4"><span class="rounded-full px-2.5 py-1 text-xs font-black {{ $slide->is_active ? 'bg-emerald-50 text-emerald-700' : 'bg-red-50 text-red-700' }}">{{ $slide->is_active ? 'Active' : 'Inactive' }}</span></td>
                            <td class="px-5 py-4">
                                <div class="flex justify-end gap-2">
                                    <a href="{{ route('admin.homepage-slides.edit', $slide) }}" class="rounded-lg border border-slate-200 px-3 py-2 text-xs font-black hover:bg-slate-50">Edit</a>
                                    <form method="POST" action="{{ route('admin.homepage-slides.toggle', $slide) }}">@csrf @method('PATCH')<button class="rounded-lg border border-slate-200 px-3 py-2 text-xs font-black hover:bg-slate-50">{{ $slide->is_active ? 'Disable' : 'Enable' }}</button></form>
                                    <form method="POST" action="{{ route('admin.homepage-slides.destroy', $slide) }}" onsubmit="return confirm('Delete this homepage slide? This action cannot be undone.')">@csrf @method('DELETE')<button class="rounded-lg border border-red-200 px-3 py-2 text-xs font-black text-red-700 hover:bg-red-50">Delete</button></form>
                                </div>
                            </td>
                        </tr>
                    @empty
