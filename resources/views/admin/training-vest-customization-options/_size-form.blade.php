...
<div class="space-y-6">
    <div class="rounded-2xl border border-blue-100 bg-blue-50 px-5 py-4 text-sm font-semibold text-blue-900">
        You are editing size options for <span class="font-black">Training Vest Customization</span>. These size groups are stored separately from all other clothing size options.
    </div>

    <x-admin.section-card title="Size Group" description="Create a reusable size group only for training vest customization.">
        <div class="grid gap-5 md:grid-cols-2">
            <label class="admin-label">Name
                <input class="admin-input" name="name" value="{{ old('name', $group->name) }}" maxlength="160" placeholder="Example: Adult Training Vest" required>
            </label>
    ...
    <x-admin.section-card title="Size Options" description="These training vest sizes become available only when this size group is used.">
    ...
    <input type="hidden" name="sort_order" value="{{ old('sort_order', $group->sort_order ?? 0) }}">
    ...
    <div class="sticky bottom-3 z-30 flex flex-col gap-3 rounded-2xl border border-slate-200 bg-white/95 p-4 shadow-soft backdrop-blur sm:bottom-4 sm:flex-row sm:justify-end">
        <a class="btn btn-white" href="{{ route('admin.training-vest-customization-options.type', $type->value) }}">Cancel</a>
