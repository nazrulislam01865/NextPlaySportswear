@php
    ...
    $selectedType = old('type', $option->type instanceof TrainingVestCustomizationType ? $option->type->value : $option->type);
    $selectedTypeEnum = TrainingVestCustomizationType::tryFrom((string) $selectedType) ?: TrainingVestCustomizationType::Color;
    $imageTitle = $selectedTypeEnum->imageTitle();
    $imageDescription = $selectedTypeEnum->imageDescription();
    $colorTypes = collect(TrainingVestCustomizationType::cases())
        ->filter(fn (TrainingVestCustomizationType $type) => $type->usesColorValue())
        ->map(fn (TrainingVestCustomizationType $type) => $type->value)
        ->values()
        ->all();
    $descriptionTypes = collect(TrainingVestCustomizationType::cases())
        ->filter(fn (TrainingVestCustomizationType $type) => $type->usesDescription())
        ->map(fn (TrainingVestCustomizationType $type) => $type->value)
        ->values()
        ->all();
    ...
    $cancelUrl = $returnType
        ? route('admin.training-vest-customization-options.type', $returnType)
        : route('admin.training-vest-customization-options.index');
...
>
    ...
    <x-admin.section-card
        ...
        description="Create a reusable training vest customization value. Images are optional."
    >
        <div class="space-y-5">
            ...
            >
                ...
                <span class="mt-1 block text-xs font-medium text-slate-500">
                    This value is shown only for training vest color options.
                </span>
            </label>
            ...
            >
                ...
                <span class="mt-1 block text-xs font-medium text-slate-500">
                    Details are shown only for training vest fabric options.
                </span>
            </label>
        </div>
    </x-admin.section-card>
