...
final class CartMutationIdempotencyService
{
    ...
    public function replay(Request $request, string $operation, array $payload): ?array
    {
        ...
        $entry = (array) $request->session()->get($this->sessionKey($request, $operation, $key), []);
        if ($entry === []) {
            return null;
        }

        if ((int) ($entry['expires_at'] ?? 0) < time()) {
            $request->session()->forget($this->sessionKey($request, $operation, $key));
            return null;
        }

        if (! hash_equals((string) ($entry['fingerprint'] ?? ''), $this->fingerprint($payload))) {
            throw new ConflictHttpException('The same Idempotency-Key cannot be reused with a different cart payload.');
        }
    ...
    private function sessionKey(Request $request, string $operation, string $key): string
    {
        ...
        $identity = $user?->isCustomer()
            ? 'customer:'.$user->getKey()
            : 'session:'.CartSessionIdentity::current($request);

        return 'nextplay_api.idempotency.'.hash('sha256', $identity.'|'.$operation.'|'.$key);
    }
