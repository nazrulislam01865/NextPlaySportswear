...
class TrainingVestCustomizationOptionRequest extends FormRequest
{
    ...
    {
        $option = $this->route('trainingVestCustomizationOption');

        return [
            'name' => ['required', 'string', 'max:160'],
            'type' => ['required', Rule::in(array_keys(TrainingVestCustomizationType::masterDataOptions()))],
            'slug' => [
                'required',
                'string',
                'max:180',
                Rule::unique('training_vest_customization_options', 'slug')
                    ->where(fn ($query) => $query->where('type', $this->input('type')))
                    ->ignore($option?->getKey()),
            ],
            'color_hex' => [
                Rule::requiredIf(TrainingVestCustomizationType::tryFrom((string) $this->input('type'))?->usesColorValue() === true),
                'nullable',
                'regex:/^#[0-9A-F]{6}$/',
            ],
    ...
    protected function prepareForValidation(): void
    {
        ...
        $typeEnum = TrainingVestCustomizationType::tryFrom($type);
        ...
        $this->merge([
            ...
            'description' => $typeEnum?->usesDescription() === true && filled($this->input('description'))
                ...
                : null,
            'is_active' => true,
            'sort_order' => $option instanceof TrainingVestCustomizationOption
