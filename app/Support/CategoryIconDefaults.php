...
final class CategoryIconDefaults
{
    public static function key(?string $label): string
    {
        ...
        if (str_contains($name, 'drink')
            || str_contains($name, 'bottle')
            || str_contains($name, 'tumbler')
            || str_contains($name, 'mug')
            || str_contains($name, 'cup')) {
            return 'drinkware';
        }

        if (str_contains($name, 'bag') || str_contains($name, 'backpack') || str_contains($name, 'drawstring')) {
            return 'bag';
        }

        if (str_contains($name, 'cap') || str_contains($name, 'head') || str_contains($name, 'hat')) {
            return 'headwear';
        }
    ...
    public static function svgPaths(?string $label): string
    {
        return match (self::key($label)) {
            'drinkware' => '<path d="M8 3h8"/><path d="M9 3v3l-1 1.5V20c0 .6.4 1 1 1h6c.6 0 1-.4 1-1V7.5L15 6V3"/><path d="M8 10h8"/><path d="M10.5 14h3"/>',
